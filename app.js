...
// Deduplicated maintenance check — all callers share one in-flight request.
// Uses /api/maintenance (server-cached, <5 ms) instead of a direct Supabase
// query so it doesn't generate extra HTTP/2 streams at startup.
let _maintPromise = null;
        ...
        _maintPromise = (async () => {
            try {
                ...
                if (r.ok) {
                    ...
                    if (d.maintenance) { window.__mozpayShowMaintenanceScreen(); return true; }
                }
            } catch (_) { /* fail-open */ }
            return false;
        })();
    }
    return _maintPromise;
};
